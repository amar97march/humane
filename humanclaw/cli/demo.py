"""HumanClaw Demo — the hook. Shows the full system working in 10 seconds."""
...
def run_demo():
    ...
    header = Text()
    header.append("══════════════════════════════════════════\n", style="yellow")
    header.append("  HUMANCLAW DEMO", style="bold white")
    ...
    console.print("[bold]Nobody asked it to. That's HumanClaw.[/]")
    console.print("────────────────────────────────────────", style="yellow")
    console.print()
    console.print("[yellow]→[/]  run [bold]'humanclaw'[/] to open the dashboard")
    console.print("[yellow]→[/]  run [bold]'humanclaw status'[/] to check agent state")
...
def _seed_demo_data(conductor: Conductor):
    ...
    conductor.memory_decay.add_memory(
        memory_type=__import__("humanclaw.core.models", fromlist=["MemoryType"]).MemoryType.EPISODIC,
